...
class ArcticBlockTracer {
	static public function hierarchyHeight(b: ArcticBlock): Int {
		if (b == null) {
			return 0;
		}
		
		var max = function(a: Int, b: Int): Int {
			return (a > b ? a : b);
		}
		
		switch (b) {
			case Background(color, block, alpha, roundRadius):
				return 1 + hierarchyHeight(block);
			...
			case Button(block, hover, onClick, onClickExtended):
				return 1 + max(hierarchyHeight(block), hierarchyHeight(hover));

			case ToggleButton(selected, unselected, initialState, onChange, onInit):
				return 1 + max(hierarchyHeight(selected), hierarchyHeight(unselected));
			...
			case ColumnStack(columns, useIntergerFillings):
				var max = -1;
				for (c in columns) {
					var h = hierarchyHeight(c);
					if (h > max) {
						max = h;
					}
				}
			...
			case LineStack(blocks, ensureVisibleIndex, useScrollbar, useIntergerFillings):
				var max = -1;
				for (c in blocks) {
					var h = hierarchyHeight(c);
					if (h > max) {
			...
			case Grid(blocks, disableScrollbar, oddRowColor, evenRowColor, borderSize, borderColor):
				var max = -1;
				for (b in blocks) {
					for (c in b) {
						var h = hierarchyHeight(c);
			...
			case ScrollBar(block, fixedWidth, fixedHeight):
				return 1 + hierarchyHeight(block);

			case Dragable(stayWithinBlockBool, sideMotionAllowed, upDownMotionAllowed, block, 
					onDrag, onInit, onStopDrag):
				return 1 + hierarchyHeight(block);

			case Cursor(block, cursor, keepNormalCursor):
				return 1 + max(hierarchyHeight(block), hierarchyHeight(cursor));

			case Offset(xOffset, yOffset, block):
				return 1 + hierarchyHeight(block);

			case OnTop(base, overlay):
				return 1 + max(hierarchyHeight(base), hierarchyHeight(overlay));
			...
			case Mask(block, mask):
				return 1 + max(hierarchyHeight(block), hierarchyHeight(mask));
			...
			case Animate(animator):
				return 1 + hierarchyHeight(animator.block);
	...
	static public function elementsCount(b: ArcticBlock): Int {
		if (b == null) {
			return 0;
		}
		...
		switch (b) {
			...
			case Button(block, hover, onClick, onClickExtended):
				return 1 + elementsCount(block) + elementsCount(hover);

			case ToggleButton(selected, unselected, initialState, onChange, onInit):
				return 1 + elementsCount(selected) + elementsCount(unselected);
			...
			case Cursor(block, cursor, keepNormalCursor):
				return 1 + elementsCount(block) + elementsCount(cursor);

			case Offset(xOffset, yOffset, block):
				return 1 + elementsCount(block);

			case OnTop(base, overlay):
				return 1 + elementsCount(base) + elementsCount(overlay);
			...
			case Mask(block, mask):
				return 1 + elementsCount(block) + elementsCount(mask);
			...
			/// An animation block
			case Animate(animator):
				return 1 + elementsCount(animator.block);
